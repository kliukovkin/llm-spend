...
class BreakdownRow:
    value: str
    cost_usd: float
    share: float  # of total cost across all rows, 0..1
...
def breakdown(records: list[UsageRecord], dimension: Dimension) -> list[BreakdownRow]:
    ...
    for r in records:
        ...
        totals[value] = totals.get(value, 0.0) + r.cost_usd
    grand_total = sum(totals.values()) or 1.0
    rows = [BreakdownRow(value=v, cost_usd=c, share=c / grand_total) for v, c in totals.items()]
    rows.sort(key=lambda row: row.cost_usd, reverse=True)
    return rows


def most_expensive_day(records: list[UsageRecord]) -> tuple[date, float] | None:
...
def top_movers(records: list[UsageRecord], dimension: Dimension, recent_days: int = 7) -> list[MoverRow]:
    ...
    for r in records:
        ...
        if recent_start <= d <= latest_date:
            recent_totals[value] = recent_totals.get(value, 0.0) + r.cost_usd
        elif previous_start <= d <= previous_end:
            previous_totals[value] = previous_totals.get(value, 0.0) + r.cost_usd

    rows = []
    for value in set(recent_totals) | set(previous_totals):
        recent_cost = recent_totals.get(value, 0.0)
        previous_cost = previous_totals.get(value, 0.0)
        delta = recent_cost - previous_cost
        pct_change = (delta / previous_cost) if previous_cost > 0 else None
        rows.append(
